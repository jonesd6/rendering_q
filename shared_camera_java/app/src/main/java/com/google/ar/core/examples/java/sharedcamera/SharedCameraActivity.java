...
public class SharedCameraActivity extends AppCompatActivity
  ...
  private int WIDTH_MAX = 4032;
  private int HEIGHT_MAX = 3024;
  ...
  public final AtomicBoolean shouldUpdateSurfaceTexture = new AtomicBoolean(false);
  ...
  protected void onResume() {
    super.onResume();
    Log.d("DEBUGGING", "SharedCameraActivity resumed.");
    waitUntilCameraCaptureSessionIsActive();
    Log.d("DEBUGGING", "Camera capture session is active.");
    startBackgroundThread();
    Log.d("DEBUGGING", "Background thread is started");
    surfaceView.onResume();
    Log.d("DEBUGGING", "Surface view is resumed.");

    //while (!renderer.surfaceCreated) {
   // }
    // When the activity starts and resumes for the first time, openCamera() will be called
    // from onSurfaceCreated(). In subsequent resumes we call openCamera() here.
    if (renderer.surfaceCreated) {
      Log.d("DEBUGGING", "surfaceCreated()");
      openCamera();
    }
  ...
  private void createCameraPreviewSession() {
    try {
      ...
      sharedSession.setCameraTextureName(renderer.backgroundRenderer.getTextureId());
  ...
  private void openCamera() {
    ...
    getResolutions(this, "0", ImageFormat.YUV_420_888);
  ...
  public void onDrawFrameARCore() throws CameraNotAvailableException {
    ...
    Log.d("DEBUGGING", "onDrawFrameARCore()");

    // Perform ARCore per-frame update.
    Frame frame = sharedSession.update();
    Log.d("DEBUGGING", "sharedSession.update()");
    Camera camera = frame.getCamera();
    Log.d("DEBUGGING", "frame.getCamera()");
    ...
    Log.d(TAG, "isGLAttached set to true.");

    // If frame is ready, render camera preview image to the GL surface.
    renderer.backgroundRenderer.draw(frame);
  ...
  public ArrayList<String> getResolutions (Context context, String cameraId,int imageFormat){
    ...
    for(String resol:output) {
      Log.d("STREAMS", String.valueOf(imageFormat) + " - - - " + resol);
    }
    return output;
  }
}
